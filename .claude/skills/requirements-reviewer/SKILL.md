...
以下の状態遷移表に基づいて完了を判定する:

| 現在の状態 | イベント | 遷移先 | 処理 |
|---|---|---|---|
| レビュー中 | `chartability: "ready"` | **完了** | 修正後のファイルを出力 |
| レビュー中 | Critical/Major指摘あり & ラウンド < 3 | レビュー中 | パッチ適用 → 再レビュー |
| レビュー中 | Critical/Major指摘あり & ラウンド = 3 | **エスカレーション** | 残存問題をユーザーに提示し手動修正を依頼 |
| レビュー中 | Minor/Info指摘のみ | **完了** | 自動適用して終了 |
| レビュー中 | ユーザーが「修正せずに続行」選択 | **完了** | 現状のまま出力 |
| エスカレーション | ユーザーが手動修正 | レビュー中 | 追加ラウンドなし、修正後のファイルを出力 |
| エスカレーション | ユーザーが「続行」選択 | **完了** | 現状のまま出力 |
